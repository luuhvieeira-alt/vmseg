        ...
        <div className="space-y-12 animate-in fade-in duration-500 max-w-[1600px] mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {usuarios.filter(u => u.setor === 'VENDEDOR').map(v => {
              ...
              return (
                <button key={v.id} onClick={() => setSelectedSellerRh(v.nome)} className="bg-[#111827] p-10 rounded-[2rem] border border-gray-800/30 text-left group hover:border-blue-500/30 transition-all shadow-2xl relative overflow-hidden">
      ...
      {activeSection === 'vendedores' && currentUserData.setor === 'ADMIN' && (
        <div className="space-y-10 max-w-[1600px] mx-auto animate-in fade-in duration-500 px-4">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
             {usuarios.map(u => (
               <div key={u.id} className="bg-[#111827] p-8 rounded-[2rem] border border-gray-800 relative group shadow-2xl transition-all hover:border-red-900/30">
        ...
        <div className="space-y-10 max-w-[1600px] mx-auto animate-in fade-in duration-500 px-4">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             {metas.map(m => {
                ...
                return (
                  <div key={m.id} className={`bg-[#111827] p-10 rounded-[2.5rem] border shadow-2xl relative overflow-hidden group transition-all ${isCompany ? 'border-purple-900/30' : 'border-blue-900/20'}`}>
          ...
          <div className="space-y-6">
            ...
            {modalType === 'cancelamento' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1"><label className="text-[9px] font-black uppercase text-gray-500 ml-2 tracking-widest">VENDEDOR</label><select className="w-full bg-[#0b0f1a] border border-gray-800 p-5 rounded-xl text-white outline-none text-xs uppercase font-black shadow-inner" value={editingItem?.vendedor || ''} onChange={e => setEditingItem({...editingItem, vendedor: e.target.value})}><option value="">SELECIONE</option>{usuarios.filter(u => u.setor === 'VENDEDOR').map(u => <option key={u.id} value={u.nome}>{u.nome.toUpperCase()}</option>)}</select></div>
